...
//		parent.setIconImage(Gossip.getImage16());
	}   //  ALogin


	protected static final String RESOURCE = "io.homebeaver.gossip.ALoginRes";
...
//	private BorderLayout defaultLayout = new BorderLayout();
	...
	private void jbInit() throws Exception
	{
		...
		titleLabel.setToolTipText(Gossip.getURL());
		titleLabel.setHorizontalTextPosition(SwingConstants.CENTER);
		titleLabel.setIcon(Gossip.getImageIconLogo());
		titleLabel.setText(Gossip.getSubtitle());
		...
		compileDate.setText(Gossip.DATE_VERSION);
		compileDate.setToolTipText(Gossip.getImplementationVendor());
		...
		versionLabel.setText(Gossip.MAIN_VERSION);
		versionLabel.setToolTipText(Gossip.getImplementationVersion());
		...
		copy1Label.setText(Gossip.COPYRIGHT);
...
//			OnlineHelp.openInDefaultBrowser();
			...
			if(Util.isEmpty(restorePasswordURL)) {
				...
				if(!Util.isEmpty(appHost)) {
					restorePasswordURL = Gossip.getWebServer(appHost);
				}
			}
			if(!Util.isEmpty(restorePasswordURL)) {
	...
	private boolean checkVersion()
	{
		...
		try
		{
			...
			if (Gossip.DATE_VERSION.equals(version))
			{
			...
			else if (version != null)
			{
				...
				msg.append(version == null ? "null" : version).append(" <> ")
					.append(Gossip.DATE_VERSION).append("\n");
				msg.append(res.getString("PleaseUpgrade")).append("\n<<");
				JOptionPane.showMessageDialog(null, msg.toString(),
						Gossip.getName() + " - " + res.getString("VersionConflict"),
